...
class MCPService:
    ...
    def create_mcp(self, db: Session, mcp_data: MCPCreate, user_id: UUID) -> MCPServer:
        data = mcp_data.dict()
    ...
    def update_mcp(self, db: Session, mcp_id: UUID, update_data: MCPUpdate, user_id: UUID):
        ...
        if env_vars is not None:
             # Logic: Filter out existing vars that are masked, only process new values or deletions
             # The frontend sends the full list of DESIRED env vars.
             # We should resync perfectly to that list.
